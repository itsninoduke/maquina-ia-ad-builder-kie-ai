# Character sheet (ChatGPT Image 2 via KIE) — generate an AI influencer

**Use when:** The user wants to create a new AI influencer using **gpt-image-2** instead of Nano Banana. Same 10-image character sheet workflow, but hits KIE's **dedicated `/api/v1/gpt4o-image/generate` endpoint** (not `/jobs/createTask`).
...
| Factor | gpt-image-2 (this file) | Nano Banana 2 / Pro |
|---|---|---|
...
| **Multi-reference blending** | Capped at **5 `filesUrl`** | Up to **14 `image_input`** — more anchors = tighter continuity |
| **Aspect ratios supported** | `1:1`, `3:2`, `2:3` only (no 9:16 / 16:9 on this endpoint) | Full Meta set (`1:1`, `4:5`, `5:4`, `2:3`, `3:2`, `9:16`, `16:9`, `3:4`, `4:3`, `21:9`) |
...
6. **Generate 9 remaining angles** using the hero + up to 4 prior angle URLs as `filesUrl` (Step 4 — note the 5-ref cap).
...
Anchor image. All 9 other angles will reference it.
...
2. Call **`POST /api/v1/gpt4o-image/generate`** (NOT `/jobs/createTask`) with:
   ```json
   {
     "prompt": "<hero prompt>",
     "size": "2:3",
   ...
   ```
   - Note: `size` (not `aspect_ratio`), `nVariants` (not `--n`), `2:3` (the closest gpt-image-2-supported ratio to the 9:16 used by Nano Banana version).
   - The agent / `chatgpt-image-ad` script handles the auth header (`Bearer $KIE_API_KEY`).

3. **Poll `GET /api/v1/gpt4o-image/record-info?taskId=<id>`** (NOT `/jobs/recordInfo`) until `data.successFlag == 1`. Result URL is at `data.response.resultUrls[0]` (camelCase — observed live; docs say `result_urls` but the API returns `resultUrls`).

4. **Post-generation QA:** Inspect for anatomy defects per [nano-banana.md § QA checklist](nano-banana.md). gpt-image-2-specific watch-outs:
...
5. Download the image to the character's `references/influencers/{folder}/`. Note: KIE result URLs are public CDN links with ~20 min expiry — download immediately. The script's `http_download` uses a Mozilla User-Agent (the CDN 403s the default Python UA).

6. **Open the image for the user** so they can review at full resolution:
   - macOS: `open <path>`
   - Linux: `xdg-open <path>`
   - Windows: `explorer <path>`

7. **Wait for explicit user approval.** This is the character — if they don't like it, iterate before generating 9 more images. Do NOT proceed without approval.

## Step 4: Generate 9 remaining angles (5-ref cap)

Once the hero is approved:

1. **Host the approved hero image at a publicly reachable HTTPS URL.** KIE has no presigned-upload flow — your `filesUrl` references must be public URLs reachable from KIE's servers. See `MASTER_CONTEXT.md` § *Image hosting* for the user's hosting strategy. If the hero just came from KIE, you can re-use that URL directly while it's still live (~20 min); after that, download + re-host.

2. **Reference-image strategy (key gpt-image-2 difference):**
   - gpt-image-2 on KIE accepts **max 5 URLs in `filesUrl`** (per the endpoint docs).
   - Always include the **hero** as the first ref (the anchor).
   - For angles 2-5: pass `[hero_url]` only (4 free slots, but you have no prior angles yet anyway).
   - For angles 6+: pass `[hero_url, angle_N-1_url, angle_N-2_url, angle_N-3_url, angle_N-4_url]` — the most recent 4 angles plus the hero. This keeps continuity from the most recent successful gen while still anchoring on the hero.
   - **Don't include angles that drifted.** If angle N looked off, exclude it from the reference set for angle N+1.
   - **Don't include angles whose KIE result URL has expired.** Result URLs are short-lived (~20 min). For long character-sheet sessions, re-host generated angles to your CDN once they're QA'd, then use those stable URLs as refs for later angles.
...
4. Call `POST /api/v1/gpt4o-image/generate` for each with:
   ```json
   {
     "prompt": "<angle-specific prompt with full character anchor>",
     "size": "2:3",
     "filesUrl": ["<hero_url>", "<recent_angle_urls...>"],
     "nVariants": 1
   }
   ```

5. Fire all 9 in sequence (not parallel — KIE rate-limits at 20 req per 10s and gpt-image-2 is on a dedicated endpoint with its own limits). Poll all concurrently.
...
- **Profile shots (4, 5)** drift the most on gpt-image-2 — facial features can shift between profile and 3/4. Pass `[hero_url, 02-3q-left_url]` as refs for `04-profile-left` (the 3/4 anchors the facial features in a related angle).
...
If any image drifts significantly, note it when presenting results. **Offer the user a "regenerate with stronger anchor" option:** rerun that angle with `[hero_url, 01-hero-front_url, 06-face-closeup_url]` (or whatever angles render most cleanly) as refs, plus an even more verbose textual character description.
...
gpt-image-2 cost on KIE varies — check `logs/kie-api.jsonl` for your recent credit values, or KIE's pricing page. Standard pattern:
...
- **Product showcase** ([product-showcase.md](product-showcase.md)) — use the hero URL + product photo URL to generate the influencer holding the product. Use `gpt-image-2` again for the showcase still if you want the rendering style to match the sheet.
...
- **Video generation** — pass the hero URL (or any angle URL) into Veo 3.1 (`imageUrls` for `REFERENCE_2_VIDEO`) or Sora 2 (`sora-2-image-to-video` with `image_urls`). Note: video models render the character with a slight rendering-style shift since they're not gpt-image-2-native; mention this to the user before they spend on video.
...
**Flow:** Generate hero (gpt-image-2 at 2:3) → user approves → host hero at public URL → generate 9 angles with `[hero_url]` + recent angle URLs as refs (max 5 total) → QA strictly for identity drift → save to `-gpt` folder → done.
